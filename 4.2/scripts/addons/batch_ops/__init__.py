...
dairin0d.load(globals(), {
    "bpy_inspect": "BlRna, BpyProp, prop, BlEnums",
    "utils_python": "PrimitiveLock",
    "utils_blender": "BlUtil, UndoBlock, NodeTreeComparer, BpyPath, IDProp, apply_modifiers, apply_constraints, get_idblock, obj_in_collection",
...
def trigger_redraw(redraw=True, update=False):
    # Note: depsgraph update is not allowed in certain contexts,
    # so we have to be careful about using it
    if redraw: BlUI.tag_redraw()
    if update: BlUtil.Depsgraph.trigger_update()
...
def object_context_menu_draw(self, context):
    ...
    self.layout.separator()
    self.layout.menu("VIEW3D_MT_batch_ops_prefs_object_context_local", text="Batch Categories")
    for batch_cls in batch_classes:
        if batch_cls.plural_up not in addon_prefs.categories: continue
        if batch_cls.plural_up not in panels_prefs.show_context_menu: continue
...
class BatchOpsPreferences:
    ...
    def categories_set(self, value):
        ...
        self.show_context_menu |= new_panels
    ...
    show_context_menu: {'MATERIALS', 'MODIFIERS', 'COLLECTIONS'} | prop("Context Menu", "Which Batch Ops categories to show in the object context menu", items=[])
    ...
    use_local_storage: {'PANELS_PIE', 'FILTERS', 'RENAME'} | prop("Store locally",
        ...
        ('PANELS_PIE', "Panels+Pie+Context menu", "Panel visibility, active pie menu category and context menu"),
    ...
    filtering_behavior: 'SIMPLE' | prop("Filter behavior", "How to interpret the search text", items=[
        ('SIMPLE', "Simple", "Search for the presence of the entire filter text"),
        ('MULTI_ANY', "Multi: Any", "Filter by multiple search strings (any can be present)"),
        ('MULTI_ALL', "Multi: All", "Filter by multiple search strings (all must be present)"),
        ('REGEX', "Regex", "Search using regular expressions"),
    ])
    filtering_separator: "," | prop("Filter separator", "The separator used to split the filter text in the Multi mode")
    ...
    show_hover_indicator: False | prop("Show hover indicator",
        "Indicate which items belong to the object under the cursor " +
        "(for performance reasons, works only in Object and Pose modes)\n" +
        "WARNING: this may interfere with 3D view's gizmos; disable this option if it causes problems")
    ...
    apply_options: {'CONVERT_TO_MESH', 'MAKE_SINGLE_USER', 'REMOVE_DISABLED', 'APPLY_SHAPE_KEYS', 'VISIBLE_ONLY'} | prop(
        "Apply options", "Apply options", items=[
    ...
    modstack_naming: 'TYPE' | prop("Naming options", "What names to show for modifiers/constraints/effects", items=[
        ('TYPE', "Type", "Type"),
        ('NAME', "Name", "Name"),
        ('SMART_NAME', "Smart name", "Name without .000-like suffix"),
    ], update=update_refresh)
    
    panel_newmod_width: 300.0 | prop("New dialog width", "Width of the New dialog", min=100)
    panel_newmod_ok: True | prop("New dialog Ok button", "Display Ok button in the New dialog")
    ...
    custom_props_grouping: 'NAME' | prop("Custom Props grouping", "How to group custom properties", items=[
        ('NAME', "Name", "Group by name only"),
        ('NAME_TYPE', "Name+Type", "Group by name and type"),
        ('NAME_TYPE_SIZE', "Name+Type+Size", "Group by name, type and array size"),
        ('NAME_SUBTYPE', "Name+Subtype", "Group by name and subtype"),
        ('NAME_SUBTYPE_SIZE', "Name+Subtype+Size", "Group by name, subtype and array size"),
    ], update=update_refresh)
    custom_props_ui_data: False | prop("UI data", "Include properties of UI-related datablocks", update=update_refresh)
    custom_props_drivers: {'RENAME'} | prop("FCurve/Driver options", "How FCurves/drivers should be affected", items=[
        ('RENAME', "Rename", "Update variables' data paths when properties are renamed"),
        ('REMOVE_AFFECTING', "Remove Affecting", "Remove FCurves which were affecting the deleted properties"),
        ('REMOVE_AFFECTED', "Remove Affected", "Remove FCurves which were affected by the deleted properties"),
        ('PRINT_AFFECTED', "Print Affected", "List the affected FCurves in the system console"),
    ], update=update_refresh)
    ...
    def draw_settings(self, context):
        ...
        subrow = col.row()
        subrow.label(text="3D view menu:", icon='BLANK1')
        subrow.menu("VIEW3D_MT_batch_ops_prefs_object_context")
        ...
        subrow = col.row()
        subrow.label(text="'New' dialog:", icon='BLANK1')
        ...
        subrow = col.row()
        subrow.label(text="Filter behavior:", icon='BLANK1')
        subrow.prop(self, "filtering_behavior", text="")
        subrow2 = subrow.row()
        subrow2.active = self.filtering_behavior.startswith('MULTI')
        subrow2.prop(self, "filtering_separator", text="")
        ...
        subrow = col.row()
        subrow.label(text="Hover indicator:", icon='BLANK1')
        subrow.prop(self, "show_hover_indicator", text=" ")
        subrow.label(text="") # dummy UI element to use up the remaining space
        ...
        subrow = col.row()
        subrow.label(text="Modifiers & such:", icon='BLANK1')
        subrow.prop_menu_enum(self, "modstack_naming", text="Naming options")
        subrow.prop_menu_enum(self, "apply_options", text="Apply options")
        ...
        subrow = col.row()
        subrow.label(text="Custom Props:", icon='BLANK1')
        subrow.prop(self, "custom_props_ui_data", text="UI data")
        subrow.prop_menu_enum(self, "custom_props_grouping", text="Grouping")
        subrow.prop_menu_enum(self, "custom_props_drivers", text="FCurve/Driver")
...
@addon.Operator(idname="batch_ops.hover_processor", label="Batch Ops Hover Processor", description="Batch Ops detection of the object under the cursor")
class BatchOpsHoverProcessor:
    def invoke(self, context, event):
        # Note: here, it's important to return {'PASS_THROUGH'} rather than anything else,
        # because otherwise tooltips would be prevented from showing up.
        
        # Doing nothing when hover indicator is disabled is a good idea anyway,
        # but it also allows users to avoid the remaining conflicts with the
        # transform gizmo (e.g. when user input is coming quickly, and the
        # event.value_prev check doesn't help)
        if not addon.preferences.show_hover_indicator: return {'PASS_THROUGH'}
        
        # Selection casting appears to disrupt some internal state
        # which Blender's transform gizmos use to decide whether they
        # should activate on user click. Luckily, it seems that we can
        # circumvent this problem by doing casting only when there were
        # no user interaction events on the previous update.
        # Without interactions, event type/value are LEFTMOUSE/RELEASE
        if event.value_prev not in ('RELEASE', 'NOTHING'): return {'PASS_THROUGH'}
        
        hover_idnames = {}
        addon.runtime.hover_idnames = hover_idnames
        
        is_object_selection = BlEnums.mode_infos[context.mode].type not in ('EDIT', 'POSE')
        if not is_object_selection: return {'PASS_THROUGH'}
        
        sv = SmartView3D(context)
        if not sv: return {'PASS_THROUGH'}
        
        mouse = Vector((event.mouse_x, event.mouse_y))
        raycast_result = sv.select(mouse, coords='WINDOW')
        obj = (raycast_result.obj if raycast_result and raycast_result.success else None)
        ...
        for batch_cls in batch_classes:
            if batch_cls.plural_up not in addon_prefs.categories: continue
            hover_idnames[batch_cls.plural_up] = set(item[0] for item in batch_cls.enumerate(objs, context=context))
...
@addon.Menu(idname="VIEW3D_MT_batch_ops_prefs_object_context", label="Context menu", description="Context menu")
class BatchOpsPrefsObjectContextMenu: # used only in addon preferences
    def draw(self, context):
        layout = self.layout
        
        addon_prefs = addon.preferences
        
        for item in BatchOpsPreferences.get_enum_items("show_context_menu"):
            if item[0] not in addon_prefs.categories: continue
            layout.prop_enum(addon_prefs, "show_context_menu", item[0])

@addon.Menu(idname="VIEW3D_MT_batch_ops_prefs_object_context_local", label="Catergories", description="Batch ops categories")
class BatchOpsPrefsObjectContextMenuLocal: # used only in addon preferences
    def draw(self, context):
        layout = self.layout
        
        addon_prefs = addon.preferences
        panel_prefs = get_screen_prefs(context, 'PANELS_PIE')
        
        for item in BatchOpsPreferences.get_enum_items("show_context_menu"):
            if item[0] not in addon_prefs.categories: continue
            layout.prop_enum(panel_prefs, "show_context_menu", item[0])
...
def make_batch_ops(batch_cls, singular, plural=None):
    ...
    if hasattr(batch_cls, "initialize"):
        batch_cls.initialize()
    ...
    for prop_name in ("categories", "show_panels", "show_context_menu", "pie_view3d", "mute_on_isolate"):
    ...
    class BatchOpsPanelBase(bpy.types.Panel):
        ...
        def draw(self, context):
            ...
            hover_idnames = addon.runtime.hover_idnames.get(batch_cls.plural_up, ())
            for idname, name, users, state, obj_sel in zip(batch_cls.idnames, batch_cls.names, batch_cls.users, batch_cls.states, batch_cls.obj_sel):
                if addon_prefs.show_hover_indicator and (idname in hover_idnames):
                    icon_value = ('selection_hover' if obj_sel else 'hover')
                else:
                    icon_value = ('selection' if obj_sel else 'blank')
                icon_value = preview_collections["item_indicator"][icon_value].icon_id
                
                selected = idname in selection_set
                subrow = sublayout.row(align=True)
                op = subrow.operator(f"{prefix}.click", text=f"{name} ({users})", depress=selected, icon_value=icon_value, translate=False)
                op.idname = idname
    ...
    def BatchOp(self, context, event):
        ...
        kwargs = dict(kwargs) # don't modify the original dict!
        execution_context = kwargs.pop("execution_context", 'EXEC_DEFAULT')
    ...
    if getattr(batch_cls, "add_new", None) and ("add_new" not in main_overrides):
        ...
        class BatchOp:
            if hasattr(batch_cls, "enum_items"):
                ...
                idname: "" | prop()
            def execute(self, context):
                # Note: empty name is interpreted by Blender as "use a default name"
    ...
    if (getattr(batch_cls, "remove", None) or getattr(batch_cls, "replace", None)) and ("remove" not in main_overrides):
        @simple_operator(f"{prefix}.remove", f"Remove {plural}", f"Remove {plural.lower()} from selected objects", poll=batch_poll)
        def BatchOp(self, context):
            batch_remove = getattr(batch_cls, "remove", None)
            with UndoBlock(f"Remove {plural}"):
                if batch_remove:
                    batch_remove(batch_cls.selected_objects(context), batch_cls.selected_idnames(context))
                else:
...
class BatchOps:
    ...
    def _map_idnames(cls, context, objs, visible_only=False):
        ...
        if not filter_name:
            filter_predicate = (lambda name: True)
        elif addon_prefs.filtering_behavior == 'REGEX':
            try:
                expr = re.compile(filter_name, flags=re.IGNORECASE)
                filter_predicate = (lambda name: expr.search(name))
            except:
                filter_predicate = (lambda name: True)
        elif addon_prefs.filtering_behavior.startswith('MULTI'):
            # If separator not specified, use whitespace (the None argument)
            filter_names = []
            for filter_name in filter_name.split(addon_prefs.filtering_separator or None):
                filter_name = filter_name.strip().lower()
                if filter_name: filter_names.append(filter_name)
            
            if not filter_names:
                filter_predicate = (lambda name: True)
            elif addon_prefs.filtering_behavior.endswith('ALL'):
                filter_predicate = (lambda name: all((filter_name in name.lower()) for filter_name in filter_names))
            else:
                filter_predicate = (lambda name: any((filter_name in name.lower()) for filter_name in filter_names))
        else:
            filter_name = filter_name.lower()
            filter_predicate = (lambda name: filter_name in name.lower())
        ...
        for idname, name, users, index, states in cls.enumerate(objs, context=context, visible_only=visible_only):
            ...
            if not filter_predicate(name): continue
...
class BatchOps_ModStack(BatchOps):
    separator = "\x1f"
    ...
    @classmethod
    def idname(cls, item):
        return cls.idname_and_name(item)[0]
    
    @classmethod
    def idname_and_name(cls, item):
        naming_mode = addon.preferences.modstack_naming
        if naming_mode == 'TYPE':
            return item.type, cls.dict_items[item.type][1]
        name = item.name
        if naming_mode == 'SMART_NAME':
            name = cls._remove_suffix(name)
        return f"{item.type}{cls.separator}{name}", name
    
    @classmethod
    def get_item_type(cls, idname):
        i = idname.find(cls.separator)
        return (idname if i < 0 else idname[:i])
    
    @classmethod
    def get_item_name(cls, idname):
        i = idname.find(cls.separator)
        return (cls.dict_items[idname][1] if i < 0 else idname[i+1:])
    
    @classmethod
    def icon_data(cls, idname):
        item = cls.dict_items.get(cls.get_item_type(idname))
    ...
    def enumerate(cls, objs, callbacks=(), context=None, visible_only=False, raw=False):
        ...
        flags = cls.toggle_enabled_flags
        
        for obj in objs:
            for i, item in enumerate(cls.get_stack(obj)):
                state = (getattr(item, flags[0]) if BlUtil.Object.select_get(obj) else None)
                idname, name = cls.idname_and_name(item)
    ...
    def filter(cls, objs, idnames, callbacks=(), context=None, visible_only=False):
        def callback(obj):
            ...
            for item in cls.get_stack(obj):
                if cls.idname(item) in idnames: return True
    ...
    def copy(cls, obj):
        if not obj:
            ...
            cls.clipboard = [BlRna.serialize(item) for item in cls.get_stack(obj)]
    ...
    def paste(cls, objs, clipboard=None, overwrite=True):
        ...
        for obj in objs:
            if overwrite: cls.stack_clear(obj)
            cls.stack_paste(obj, 'AFTER', *clipboard)
    
    @classmethod
    def add_new(cls, objs, type_name):
        type_name = cls.get_item_type(type_name) # just in case
        for obj in objs:
            cls.stack_paste(obj, 'AFTER', dict(name=cls.dict_items[type_name][1], type=type_name))
    ...
    def assign(cls, objs, idnames, overwrite=True):
        ...
        for idname in idnames:
            if not idname:
                ...
                clipboard.append(dict(name=cls.get_item_name(idname), type=cls.get_item_type(idname)))
        cls.paste(objs, clipboard, overwrite=overwrite)
    
    @classmethod
    def replace(cls, objs, idnames, idname):
        item_info = (dict(name=cls.get_item_name(idname), type=cls.get_item_type(idname)) if idname else None)
        for obj in objs:
            for i, item in enumerate(tuple(cls.get_stack(obj))):
                if cls.idname(item) not in idnames: continue
                cls.stack_paste(obj, item, item_info)
    ...
    def rename(cls, idnames, name):
        ...
        for obj in objs:
            for i, item in enumerate(tuple(cls.get_stack(obj))):
                idname = cls.idname(item)
                if idname not in idnames: continue
                item.name = names[idnames.index(idname)]
                items_map.setdefault(idname, []).append(item)
        return {idname: [cls.idname(item) for item in items] for idname, items in items_map.items()}
    ...
    def sync_editor(cls, context, area):
        ...
        if area.type == 'PROPERTIES':
            try:
                space.context = cls.area_space_context
            except TypeError:
                return
    
    @classmethod
    def stack_paste(cls, obj, where, *item_infos):
        replace_item = (None if (not where) or isinstance(where, str) else where)
        
        insert_index = -1
        if replace_item:
            insert_index = cls.stack_index(obj, replace_item)
            if insert_index < 0: return
            cls.stack_remove(obj, replace_item)
        elif where == 'BEFORE':
            insert_index = 0
        
        for item_info in item_infos:
            if not item_info: continue
            item = cls.stack_new(obj, item_info["name"], item_info["type"])
            if not item: continue
            BlRna.deserialize(item, item_info, ignore_default=True, suppress_errors=True)
            if insert_index >= 0:
                cls.stack_move(obj, item, insert_index)
                insert_index += 1
    
    @classmethod
    def stack_new(cls, obj, name, item_type):
        # Note: constraints.new() has a different signature. We have to
        # rely on exceptions since the inspect module doesn't work on it
        stack = cls.get_stack(obj)
        try:
            return stack.new(name, item_type)
        except TypeError:
            item = stack.new(item_type)
            item.name = name
            return item
    
    @classmethod
    def stack_index(cls, obj, item):
        stack = cls.get_stack(obj)
        if isinstance(item, str):
            ...
            for i, item2 in enumerate(stack):
                if item2 == item: return i
        return -1
    
    @classmethod
    def stack_move(cls, obj, item, to_index):
        stack = cls.get_stack(obj)
        
        if isinstance(item, int):
            from_index = item
            name = stack[from_index].name
        else:
            name = (item if isinstance(item, str) else item.name)
            from_index = cls.stack_index(obj, name)
        
        if hasattr(stack, "move"):
            if from_index >= 0: stack.move(from_index, to_index)
            return
        ...
        kwargs = cls.get_op_kwargs(name)
        move_to_index = cls.get_op_func("move_to_index")
        
        if move_to_index:
            kwargs["index"] = to_index
            move_to_index(**kwargs)
        elif to_index < from_index:
            move_up = cls.get_op_func("move_up")
            for _ in range(to_index, from_index):
                move_up(**kwargs)
        elif to_index > from_index:
            move_down = cls.get_op_func("move_down")
    ...
    @classmethod
    def stack_remove(cls, obj, item):
        stack = cls.get_stack(obj)
        stack.remove(item)
    
    @classmethod
    def stack_clear(cls, obj):
        stack = cls.get_stack(obj)
        stack.clear()
    
    @classmethod
    def get_op_idname(cls, op_name):
        op_parts = cls.stack_ops_info.get(op_name, None)
        if not op_parts: return None
        return ".".join(op_parts)
    
    @classmethod
    def get_op_func(cls, op_name):
        op_parts = cls.stack_ops_info.get(op_name, None)
        if not op_parts: return None
        category = getattr(bpy.ops, op_parts[0], None)
        if not category: return None
        return getattr(category, op_parts[1], None)
    
    @classmethod
    def get_op_kwargs(cls, name):
        # Defaults are only necessary for constraints (the "owner" argument)
        # and only for operators other than add (i.e. in the "constraints" category)
        kwargs = dict(cls.stack_ops_defaults)
        kwargs[cls.stack_ops_argname] = name
        return kwargs
    
    @classmethod
    def initialize(cls):
        if not hasattr(cls, "type_rna_prop"):
            cls.type_rna_prop = cls.get_op_func("add").get_rna_type().properties["type"]
        
        cls.enum_items = [(item.identifier, item.name, item.description, item.icon, item.value)
            for item in cls.type_rna_prop.enum_items]
        
        cls.dict_items = {item[0]:item for item in cls.enum_items}
    
    @classmethod
    def draw_prefs(cls, layout, context):
        addon_prefs = addon.preferences
        
        if hasattr(cls, "apply") and (cls.area_space_context == 'MODIFIER'):
            layout.prop_menu_enum(addon_prefs, "apply_options", text="Apply options")
        
        layout.prop_menu_enum(addon_prefs, "modstack_naming", text="Naming options")
    ...
    def extra_operators(cls):
        ...
        def make_enable_disable(value):
            ...
            for key_modifiers, option_name, option_flags in cls.enable_disable_options:
                key_modifiers = key_modifiers.split("+")
                keys = "+".join([key.capitalize() for key in key_modifiers if key] + ["Click"])
                key_descriptions.append(f"{keys}: {option_name}")
                
                is_alt = "alt" in key_modifiers
                is_ctrl = "ctrl" in key_modifiers
                is_shift = "shift" in key_modifiers
                key_flags[(is_alt, is_ctrl, is_shift)] = option_flags
                
                for flag in option_flags:
                    if flag not in all_flags:
                        all_flags.append(flag)
            
            description = f"{op_name} selected {cls.plural.lower()}"
            if len(key_descriptions) > 1: description += " (" + ", ".join(key_descriptions) + ")"
            
            @simple_operator(f"{cls.prefix}.{op_name.lower()}", f"{op_name} {cls.plural}", description, poll=batch_poll)
            class BatchOp:
                flags: set() | prop(items=[(flag, flag, flag) for flag in all_flags])
                ...
                def invoke(self, context, event):
                    self.flags = key_flags.get((event.alt, event.ctrl, event.shift), None)
                    if not self.flags: return {'CANCELLED'}
                    return self.execute(context)
        
        make_enable_disable(True)
        make_enable_disable(False)
        
        def make_toggle_enabled():
            description = f"Toggle enabled state for selected {cls.plural.lower()}"
            
            @simple_operator(f"{cls.prefix}.toggle_enabled", f"Toggle {cls.plural} Enabled", description, poll=batch_poll)
            def BatchOp(self, context):
                objs = cls.selected_objects(context)
                idnames = cls.selected_idnames(context)
                value = None
                ...
                for obj in objs:
                    for item in cls.get_stack(obj):
                        if cls.idname(item) not in idnames: continue
                        value = (True if value is None else value) and getattr(item, flags[0])
        ...
        def make_apply():
            apply_modes = getattr(cls, "apply_modes", None)
            
            if apply_modes and (len(apply_modes) > 1):
                modes_description = f"Click: {apply_modes[0][1]}, Alt+Click: {apply_modes[1][1]}"
            else:
                modes_description = "Click: on selection"
                apply_modes = [('APPLY', "Apply")]
            
            description = f"Apply selected {cls.plural.lower()} ({modes_description}, Ctrl+Click: on all objects in the active view layer)"
            
            @simple_operator(f"{cls.prefix}.apply", f"Apply {cls.plural}", description, poll=batch_poll)
            class BatchOp:
                mode: apply_modes[0][0] | prop(items=[(key, key, key) for key, name in apply_modes])
                globally: False | prop()
                
                @classmethod
                def poll(op_cls, context):
                    return cls.apply_poll(context)
                
                def execute(self, context):
                    addon_prefs = addon.preferences
                    options = set(addon_prefs.apply_options)
                    if self.globally: options.add('GLOBALLY')
                    ...
                    with UndoBlock(f"Apply {cls.plural}"):
                        cls.apply(objs, idnames, options=options, mode=self.mode)
                ...
                def invoke(self, context, event):
                    self.mode = (apply_modes[-1][0] if event.alt else apply_modes[0][0])
                    self.globally = event.ctrl
                    return self.execute(context)
        ...
        class BatchOps_Add_New:
            prepend: False | prop(description=f"Add to the top or to the bottom of the {cls.singular.lower()} stack")
            ...
            def _make_CURVES(self):
                return bpy.data.hair_curves.new(".tmp-curves")
            
            def _make_LATTICE(self):
                return bpy.data.lattices.new(".tmp-lattice")
            
            def _make_GPENCIL(self):
                return bpy.data.grease_pencils.new(".tmp-grease_pencil")
            
            def _make_ARMATURE(self):
                # Bones can be added/removed only via edit_bones,
                # but accessing them requires to be in EDIT mode.
                # To avoid a whole lot of extra object creation
                # and mode switching, here we utilize one of the
                # existing armatures.
                candidate_bones = None
                candidate_data = None
                for data in bpy.data.armatures:
                    data_bones = len(data.bones)
                    if data_bones == 0: continue
                    if (candidate_bones is None) or (data_bones < candidate_bones):
                        candidate_data = data
                        candidate_bones = data_bones
                if not candidate_data: return None
                data = candidate_data.copy()
                data.name = ".tmp-armature"
                data.bones.active = data.bones[0]
                data.bones[0].select = True
                return data
            
            def _remove_MESH(self, data):
                bpy.data.meshes.remove(data)
            
            def _remove_CURVE(self, data):
                bpy.data.curves.remove(data)
            
            def _remove_CURVES(self, data):
                bpy.data.hair_curves.remove(data)
            
            def _remove_LATTICE(self, data):
                bpy.data.lattices.remove(data)
            
            def _remove_GPENCIL(self, data):
                bpy.data.grease_pencils.remove(data)
            
            def _remove_ARMATURE(self, data):
                bpy.data.armatures.remove(data)
            
            def add_tmp_obj(self, context, obj_type):
                data_maker = getattr(self, f"_make_{obj_type}", None)
                data = (data_maker() if data_maker else None)
                
                obj = bpy.data.objects.new(f".tmp-{obj_type}", data)
                # Note: we cannot hide the object, or the *_add()
                ...
                self.prev_obj_mode = None
                if obj.type == 'ARMATURE':
                    self.prev_obj_mode = (self.active_obj.mode if self.active_obj else 'OBJECT')
            ...
            def del_tmp_obj(self):
                ...
                data_remover = getattr(self, f"_remove_{obj_type}", None)
                if data_remover: data_remover(data)
                self.obj = None
            
            def execute(self, context):
                if not self.obj: return {'CANCELLED'}
                
                item_infos = [BlRna.serialize(item) for item in cls.get_stack(self.obj)]
                
                # restore the initial state before recording an Undo step
                self.cleanup(context)
                
                where = ('BEFORE' if self.prepend else 'AFTER')
                
                with UndoBlock(f"Add {cls.singular}"):
                    for obj in self.objs:
                        cls.stack_paste(obj, where, *item_infos)
            ...
            def cleanup(self, context):
                ...
                if self.prev_obj_mode and bpy.ops.object.mode_set.poll():
                    bpy.ops.object.mode_set(mode=self.prev_obj_mode)
            ...
            def invoke(self, context, event):
                ...
                obj_type = cls.get_common_obj_type(self.objs)
                ...
                trigger_redraw() # make sure Properties editors are updated
            ...
            def draw(self, context):
                ...
                def draw_op(layout, op_name, icon, item):
                    kwargs = cls.get_op_kwargs(item.name)
                    op = layout.operator(cls.get_op_idname(op_name), text="", icon=icon)
                    for key, value in kwargs.items(): setattr(op, key, value)
                ...
                with layout.row():
                    ...
                    layout.operator_menu_enum(cls.get_op_idname("add"), "type")
                
                # Since Blender 2.90, there are no layout.template_*(...) methods that
                # draw individual stack items. There are methods to draw all at once,
                # but they work *exclusively* in the PROPERTIES area/region's main UI.
                
                option_names = [name for name, predicate in cls.viaibility_draw_options
                    if (not predicate) or predicate(self.obj)]
                
                for item in cls.get_stack(self.obj):
                    with layout.box():
                        with layout.row(align=True):
                            # For show_expanded, Blender automatically shows the state-dependent icon
                            layout.prop(item, "show_expanded", text="", emboss=False)
                            layout.label(icon=cls.dict_items[item.type][3])
                            layout.prop(item, "name", text="")
                            
                            for option_name in option_names:
                                layout.prop(item, option_name, text="")
                            
                            draw_op(layout, "move_up", 'TRIA_UP', item)
                            draw_op(layout, "move_down", 'TRIA_DOWN', item)
                            draw_op(layout, "copy", 'DUPLICATE', item)
                            draw_op(layout, "remove", 'X', item)
                        
                        if not item.show_expanded: continue
                        
                        base_properties = item.bl_rna.base.properties
                        
                        for prop_name, bpy_prop in BlRna.properties(item):
                            if base_properties.find(prop_name) >= 0: continue
...
class BatchOps_Modifiers(BatchOps_ModStack):
    icon = 'MODIFIER_DATA'
    ...
    contextual_ops[(1, False, True, False)] = ("apply", {"mode": 'DATA', "globally": False})
    
    area_space_context = 'MODIFIER'
    
    stack_ops_argname = "modifier"
    stack_ops_defaults = {}
    stack_ops_info = {
        "add": ("object", "modifier_add"),
        "move_up": ("object", "modifier_move_up"),
        "move_down": ("object", "modifier_move_down"),
        "move_to_index": ("object", "modifier_move_to_index"),
        "copy": ("object", "modifier_copy"),
        "remove": ("object", "modifier_remove"),
    }
    
    viaibility_draw_options = [
        ("use_apply_on_spline", (lambda obj: obj.type in ('CURVE', 'SURFACE', 'FONT'))),
        ("show_on_cage", (lambda obj: obj.type == 'MESH')),
    ...
    enable_disable_options = [
        ...
        ("ctrl", "rendering", {"show_render"}),
        ("shift", "edit mode", {"show_in_editmode"}),
        ("alt", "cage/spline", {"show_on_cage", "use_apply_on_spline"}),
    ]
    toggle_enabled_flags = ["show_viewport", "show_render"]
    ...
    @classmethod
    def get_stack(cls, obj):
        return obj.modifiers
    
    @classmethod
    def apply_poll(cls, context):
        # Modifiers cannot be applied in edit/sculpt/paint modes
        mode_info = BlEnums.mode_infos.get(context.mode)
        if not mode_info: return True
        return mode_info.type not in ('EDIT', 'SCULPT', 'PAINT')
    
    @classmethod
    def apply(cls, objs, idnames, options=(), mode='DATA'):
        validate_idname = (lambda item: cls.idname(item) in idnames)
        apply_modifiers(bpy.context, objs, validate_idname, options=options, apply_as=mode)
    
    @classmethod
    def get_common_obj_type(cls, objs):
        # MESH has all modifiers
        # CURVE, SURFACE, FONT have the same set of (fewer) modifiers
        # LATTICE has the fewest modifiers
        # CURVES has only the Geometry Nodes modifier
        obj_types = {obj.type for obj in objs}
        for obj_type in ('MESH', 'SURFACE', 'CURVE', 'FONT', 'LATTICE', 'CURVES'):
            if obj_type in obj_types: return obj_type
        return None
...
make_batch_ops_objdata('CURVES', "hair_curves", "", 'CURVES_DATA', "Hair Curve", "Hair Curves")

# =========================================================================== #

class BatchOps_NodeTrees(BatchOps_IDBlock):
    icon = 'NODETREE'
    
    @classmethod
    def icon_data(cls, idname):
        node_tree = get_idblock(bpy.data.node_groups, idname)
        return node_tree.bl_icon
    ...
    def find_in_obj(cls, obj):
        for ms in obj.material_slots:
            ...
            for node in node_tree.nodes:
                ...
                if not node_tree: continue
                yield node, "node_tree", node_tree
        
        for source, attr_name, tex in BatchOps_Textures.textures_meta(obj):
        ...
        for md in obj.modifiers:
            node_tree = getattr(md, "node_group", None)
            if not node_tree: continue
            yield md, "node_group", node_tree
    
    @classmethod
    def enumerate(cls, objs, callbacks=(), context=None, visible_only=False, raw=False):
        if objs is None:
            for node_tree in bpy.data.node_groups:
                yield (node_tree.name_full, node_tree.name_full, node_tree.users, 1, None)
        else:
            if not raw: objs = filter_objs(objs, *callbacks, context=context, subobjs=True, visible_only=visible_only)
            for obj in objs:
                for source, attr_name, node_tree in cls.find_in_obj(obj):
                    yield (node_tree.name_full, node_tree.name_full, 1, 1, None)
    ...
    def filter(cls, objs, idnames, callbacks=(), context=None, visible_only=False):
        def callback(obj):
            if isinstance(idnames, AnyIDNames): return True
            for source, attr_name, node_tree in cls.find_in_obj(obj):
                if node_tree.name_full in idnames: return True
        return filter_objs(objs, callback, *callbacks, context=context, visible_only=visible_only)
    
    @classmethod
    def remap(cls, idnames, idname):
        # We have to use a custom remap here, since we can't
        # remap different types of node trees to one another
        bpy_data = bpy.data.node_groups
        new_id = get_idblock(bpy_data, idname)
        if not new_id: return # user_remap() argument cannot be None
        for idname0 in idnames:
            idblock = get_idblock(bpy_data, idname0)
            if not idblock: continue
            if idblock.bl_idname != new_id.bl_idname: continue
            idblock.user_remap(new_id)
    
    @classmethod
    def replace(cls, objs, idnames, idname):
        repl_tree = (get_idblock(bpy.data.node_groups, idname) if idname else None)
        
        def can_replace(node_tree):
            if node_tree.name_full == idname: return False
            if node_tree.name_full not in idnames: return False
            return (not repl_tree) or (node_tree.bl_idname == repl_tree.bl_idname)
        
        if (objs is None) and repl_tree:
            for node_tree in bpy.data.node_groups:
                if not can_replace(node_tree): continue
                node_tree.user_remap(repl_tree) # user_remap does not support None argument
        else:
            if objs is None: objs = bpy.data.objects
            for obj in objs:
                for source, attr_name, node_tree in cls.find_in_obj(obj):
                    if not can_replace(node_tree): continue
                    setattr(source, attr_name, repl_tree)
    
    @classmethod
    def merge_identical(cls, idnames):
        merge_identical_idblocks(bpy.data.node_groups, idnames, compare=NodeTreeComparer.compare)

make_batch_ops(BatchOps_NodeTrees, "Node Tree")
...
class BatchOps_DataLayer(BatchOps):
    ...
    def paste(cls, objs, clipboard=None, overwrite=True):
        ...
        for obj in objs:
            ...
            if overwrite:
                cls._remove_matches(data_layers, lambda data_layer: data_layer.name not in idnames)
    ...
    def _remove(cls, objs, idnames):
        for obj in objs:
            ...
            cls._remove_matches(data_layers, lambda data_layer: data_layer.name in idnames)
            
            obj.update_tag(refresh={'OBJECT'})
    
    @classmethod
    def _remove_matches(cls, data_layers, predicate):
        # Since some Blender version, it has a weird bug - after removing
        # a data layer, other layers point to something completely different.
        # E.g. if a mesh had 2 UV layers ("UVMap", "UVMap.001"), then after
        # removing item 0, item 1 will point to "NGon Face-Vertex" instead.
        for i in range(len(data_layers) - 1, -1, -1):
            if predicate(data_layers[i]):
                data_layers.remove(data_layers[i])
...
class BatchOps_VertexColors(BatchOps_DataLayer):
    ...
    @classmethod
    def _get_layers(cls, mesh):
        return mesh.color_attributes
    
    @classmethod
    def _set_active(cls, mesh, idname, mode):
        for i, attribute in enumerate(mesh.color_attributes):
            if attribute.name == idname:
                prop_names = {'EDITING':"active_color_index", 'RENDERING':"render_color_index"}
                setattr(mesh.color_attributes, prop_names[mode], i)
                break
    
    @classmethod
    def get_layers(cls, obj):
        return (cls._get_layers(obj.data) if obj.type == 'MESH' else None)
        ...
        class BatchOp:
            ...
            def execute(self, context):
                ...
                with UndoBlock(f"Set {cls.singular} Active"):
                    for obj in cls.filter(context.scene.objects, idnames, context=context):
                        if obj.type != 'MESH': continue
                        cls._set_active(obj.data, active_idname, self.mode)
                trigger_redraw()
                return {'FINISHED'}
            def invoke(self, context, event):
                self.mode = ('RENDERING' if event.ctrl else ('CLONING' if event.alt else 'EDITING'))
...
class BatchOps_GreasePencilModifiers(BatchOps_ModStack):
    icon = 'MODIFIER_DATA'
    ...
    stack_ops_defaults = {}
    stack_ops_info = {
        "add": ("object", "gpencil_modifier_add"),
        "move_up": ("object", "gpencil_modifier_move_up"),
        "move_down": ("object", "gpencil_modifier_move_down"),
        "move_to_index": ("object", "gpencil_modifier_move_to_index"),
        "copy": ("object", "gpencil_modifier_copy"),
        "remove": ("object", "gpencil_modifier_remove"),
    }
    ...
    def apply(cls, objs, idnames, options=(), mode='DATA'):
        validate_idname = (lambda item: cls.idname(item) in idnames)
        apply_modifiers(bpy.context, objs, validate_idname, options=options, apply_as=mode, which='GPENCIL')
    
    @classmethod
    def get_common_obj_type(cls, objs):
        obj_types = {obj.type for obj in objs}
        return ('GPENCIL' if 'GPENCIL' in obj_types else None)

make_batch_ops(BatchOps_GreasePencilModifiers, "GPencil Modifier")

# =========================================================================== #

class BatchOps_ShaderEffects(BatchOps_ModStack):
    icon = 'SHADERFX'
    
    contextual_ops = dict(BatchOps.contextual_ops) # (clicks, alt, ctrl, shift)
    contextual_ops["description"] += ", Shift+Click: enable/disable"
    contextual_ops[(1, False, False, True)] = ("toggle_enabled", {})
    
    area_space_context = 'SHADERFX'
    
    stack_ops_argname = "shaderfx"
    stack_ops_defaults = {}
    stack_ops_info = {
        "add": ("object", "shaderfx_add"),
        "move_up": ("object", "shaderfx_move_up"),
        "move_down": ("object", "shaderfx_move_down"),
        "move_to_index": ("object", "shaderfx_move_to_index"),
        "copy": ("object", "shaderfx_copy"),
        "remove": ("object", "shaderfx_remove"),
    }
...
make_batch_ops(BatchOps_ShaderEffects, "Shader Effect")

# =========================================================================== #

class BatchOps_Constraints(BatchOps_ModStack):
    icon = 'CONSTRAINT'
    
    contextual_ops = dict(BatchOps.contextual_ops) # (clicks, alt, ctrl, shift)
    contextual_ops["description"] += ", Shift+Click: enable/disable, Ctrl+Click: apply"
    contextual_ops[(1, False, False, True)] = ("toggle_enabled", {})
    contextual_ops[(1, False, True, False)] = ("apply", {"mode": 'DELETE', "globally": False})
    
    # Currently, object.constraint_add() operator has no properties for the "type" argument's enum items
    type_rna_prop = bpy.types.Constraint.bl_rna.properties["type"]
    
    area_space_context = 'CONSTRAINT'
    
    stack_ops_argname = "constraint"
    stack_ops_defaults = {"owner": 'OBJECT'}
    stack_ops_info = {
        "add": ("object", "constraint_add"),
        "move_up": ("constraint", "move_up"),
        "move_down": ("constraint", "move_down"),
        "move_to_index": ("constraint", "move_to_index"),
        "copy": ("constraint", "copy"),
        "remove": ("constraint", "delete"),
    }
    ...
    toggle_enabled_flags = ["enabled"]
    
    apply_modes = [
        ('DELETE', "then delete"),
        ('DISABLE', "then disable"),
    ]
    ...
    @classmethod
    def apply(cls, objs, idnames, options=(), mode='DELETE'):
        validate_idname = (lambda item: cls.idname(item) in idnames)
        apply_constraints(bpy.context, objs, validate_idname, owner='OBJECT', mode=mode)
    
    @classmethod
    def get_common_obj_type(cls, objs):
        return 'EMPTY'

make_batch_ops(BatchOps_Constraints, "Constraint")

# =========================================================================== #

class BatchOps_BoneConstraints(BatchOps_ModStack):
    ...
    area_space_context = 'BONE_CONSTRAINT'
    
    stack_ops_argname = "constraint"
    stack_ops_defaults = {"owner": 'BONE'}
    stack_ops_info = {
        "add": ("pose", "constraint_add"),
        "move_up": ("constraint", "move_up"),
        "move_down": ("constraint", "move_down"),
        "move_to_index": ("constraint", "move_to_index"),
        "copy": ("constraint", "copy"),
    ...
    @classmethod
    def _iterate_bones(cls, obj):
        if (not obj) or (obj.type != 'ARMATURE'): return
        use_selected = (obj.mode == 'POSE')
        for pose_bone in obj.pose.bones:
            if use_selected and not pose_bone.bone.select: continue
            yield pose_bone
    
    @classmethod
    def _stack_paste(cls, pose_bone, where, item_infos):
        constraints = pose_bone.constraints
        
        def stack_index(item):
            for i, item2 in enumerate(constraints):
                if item2 == item: return i
            return -1
        ...
        insert_index = -1
        replace_item_found = False
        
        if replace_item:
            insert_index = stack_index(replace_item)
            if insert_index < 0: return False
            replace_item_found = True
            constraints.remove(replace_item)
        elif where == 'BEFORE':
            insert_index = 0
        ...
        for item_info in item_infos:
            ...
            item = constraints.new(item_info["type"])
            if not item: continue
            
            BlRna.deserialize(item, item_info, ignore_default=True, suppress_errors=True)
            
            if (insert_index >= 0) and hasattr(constraints, "move"):
                constraints.move(stack_index(item), insert_index)
                insert_index += 1
        
        return replace_item_found
    
    @classmethod
    def get_stack(cls, obj):
        constraints = []
        for pose_bone in cls._iterate_bones(obj):
            constraints.extend(pose_bone.constraints)
        return constraints
    
    @classmethod
    def stack_paste(cls, obj, where, *item_infos):
        for pose_bone in cls._iterate_bones(obj):
            if cls._stack_paste(pose_bone, where, item_infos): break
    ...
    @classmethod
    def apply(cls, objs, idnames, options=(), mode='DELETE'):
        bones = ('ALL' if ('GLOBALLY' in options) or (bpy.context.mode != 'POSE') else 'SELECTED')
        validate_idname = (lambda item: cls.idname(item) in idnames)
        apply_constraints(bpy.context, objs, validate_idname, owner='BONE', mode=mode, bones=bones)
    
    @classmethod
    def get_common_obj_type(cls, objs):
        obj_types = {obj.type for obj in objs}
        return ('ARMATURE' if 'ARMATURE' in obj_types else None)

make_batch_ops(BatchOps_BoneConstraints, "Bone Constraint")

# =========================================================================== #

class BatchOps_CustomProps(BatchOps):
    icon = 'PROPERTIES'
    
    contextual_ops = dict(BatchOps.contextual_ops) # (clicks, alt, ctrl, shift)
    contextual_ops["description"] += ", Shift+Click: edit globally, Ctrl+Click: edit selected"
    contextual_ops[(1, False, False, True)] = ("edit", {"execution_context": 'INVOKE_DEFAULT', "globally": True})
    contextual_ops[(1, False, True, False)] = ("edit", {"execution_context": 'INVOKE_DEFAULT', "globally": False})
    
    separator = " ‣ "
    
    all_id_datas = None
    ui_id_datas = {"window_managers", "workspaces", "screens"}
    ...
    class PostprocessingQueue:
        ...
        def add(self, container, key, value=None):
            if key not in container: return
            idblock = container.id_data
            data_path = f'["{key}"]'
            if container is not idblock:
                data_path = container.path_from_id() + data_path
            self.entries[(idblock, data_path)] = value
    ...
    @classmethod
    def idname(cls, container, key, grouping=None):
        grouping = grouping or addon.preferences.custom_props_grouping
        if grouping == 'NAME': return key
        
        type_name = None
        
        if 'SUBTYPE' in grouping:
            metadata = IDProp.get_metadata(container, key)
            if metadata: type_name = cls.subtype_map.get(metadata.get("subtype"))
        
        if not type_name:
            value = container[key]
            type_name, size = IDProp.get_type_info(value, True)
            if size > 0: type_name += (f"[{size}]" if 'SIZE' in grouping else "[]")
        
        return f"{key}{cls.separator}{type_name}"
    
    @classmethod
    def _remove_type(cls, name):
        sep = cls.separator
        i = name.find(sep)
        if i < 0:
            sep = sep.strip()
    ...
    def base_name(cls, id, default=None):
        if isinstance(id, int):
            ...
            return default
        return cls._remove_type(id)
    
    @classmethod
    def candidates_idblock(cls, idblock):
        yield idblock
        
        if isinstance(idblock, bpy.types.Object):
            if idblock.pose: yield from idblock.pose.bones
        elif isinstance(idblock, bpy.types.Armature):
            yield from idblock.bones
        elif isinstance(idblock, bpy.types.Scene):
            yield from idblock.view_layers
    
    @classmethod
    def candidates(cls, objs):
        if objs is None:
            if not cls.all_id_datas:
                cls.all_id_datas = []
                for name in dir(bpy.data):
                    if name in ("bl_rna", "rna_type"): continue
                    d = getattr(bpy.data, name)
                    if not hasattr(d, "foreach_get"): continue
                    cls.all_id_datas.append(name)
            
            addon_prefs = addon.preferences
            show_ui_data = addon_prefs.custom_props_ui_data
            
            for name in cls.all_id_datas:
                if (not show_ui_data) and (name in cls.ui_id_datas): continue
                for idblock in getattr(bpy.data, name):
                    if not hasattr(idblock, "items"): break
                    yield from cls.candidates_idblock(idblock)
        elif isinstance(objs, bpy.types.Object):
            obj = objs
            yield from cls.candidates_idblock(obj)
            if obj.data: yield from cls.candidates_idblock(obj.data)
        else:
            for obj in objs:
                yield from cls.candidates_idblock(obj)
                if obj.data: yield from cls.candidates_idblock(obj.data)
    ...
    @classmethod
    def prop_infos(cls, objs):
        show_developer_ui = bpy.context.preferences.view.show_developer_ui
        grouping = addon.preferences.custom_props_grouping
        
        for container in cls.candidates(objs):
            rna_props = container.bl_rna.properties
            
            for key, value in container.items():
                # Adapted from Blender's rna_prop_ui.py
                if not show_developer_ui:
                    rna_prop = rna_props.get(key)
                    if rna_prop and rna_prop.is_runtime: continue
                
                idname = cls.idname(container, key, grouping)
                yield (container, idname, key, value)
    
    @classmethod
    def enumerate(cls, objs, callbacks=(), context=None, visible_only=False, raw=False):
        if (objs is not None) and not raw:
            objs = filter_objs(objs, *callbacks, context=context, subobjs=True, visible_only=visible_only)
        
        for container, idname, key, value in cls.prop_infos(objs):
            yield (idname, idname, 1, 1, None)
    
    @classmethod
    def filter(cls, objs, idnames, callbacks=(), context=None, visible_only=False):
        def callback(obj):
            if isinstance(idnames, AnyIDNames): return True
            for container, idname, key, value in cls.prop_infos(obj):
                if idname in idnames: return True
        return filter_objs(objs, callback, *callbacks, context=context, visible_only=visible_only)
    ...
    def copy(cls, obj):
        ...
        # Copy/paste is intended to be used with objects, so we ignore
        # any other property containers associated with the object
        for key, value in obj.items():
            cls.clipboard.append((key, IDProp.get_data(obj, key)))
    ...
    def paste(cls, objs, clipboard=None, overwrite=True):
        ...
        for obj in objs:
            for info in clipboard:
                if not info: continue
                key, data = info
                if (not overwrite) and (key in obj): continue
                IDProp.set_data(obj, key, data)
            # Trigger re-evaluation of drivers
            obj.update_tag()
        
        trigger_redraw(update=True)
    
    @classmethod
    def add_new(cls, objs, name):
        # There's not much use in creating properties with some default name,
        # so if the user left the name field blank, most likely it was a mistake
        if name: cls.paste(objs, [(name, dict(value=1.0))], overwrite=False)
    
    @classmethod
    def remove(cls, objs, idnames):
        drivers_options = addon.preferences.custom_props_drivers
        remove_affecting = ('REMOVE_AFFECTING' in drivers_options)
        remove_affected = ('REMOVE_AFFECTED' in drivers_options)
        print_affected = ('PRINT_AFFECTED' in drivers_options)
        use_postprocess = remove_affecting or remove_affected or print_affected
        
        if use_postprocess:
            queue = cls.PostprocessingQueue()
        
        keys = [cls._remove_type(idname) for idname in idnames]
        for container in cls.candidates(objs):
            for key in keys:
                if use_postprocess: queue.add(container, key)
                container.pop(key, None)
        
        if use_postprocess:
            print_results = []
            
            for fcurve_info, target_info in BlUtil.FCurve.find_dependencies(queue.entries):
                fcurve = fcurve_info["fcurve"]
                
                variable = target_info.get("variable")
                if variable:
                    if print_affected:
                        idblock = fcurve_info["id"]
                        data_path = fcurve.data_path
                        if data_path and (data_path[0] not in ".["): data_path = "." + data_path
                        track = fcurve_info.get("track")
                        strip = fcurve_info.get("strip")
                        nla_info = (f" (track: {track.name}, strip: {strip.name})" if strip else "")
                        print_results.append(f"{repr(idblock)}{data_path}{nla_info}")
                    
                    if not remove_affected: continue
                    
                    #fcurve.driver.variables.remove(variable)
                    #continue
                else:
                    if not remove_affecting: continue
                
                strip = fcurve_info.get("strip")
                if strip:
                    fcurve_info["track"].strips.remove(strip)
                    continue
                
                idblock = fcurve_info["id"]
                anim_data = getattr(idblock, "animation_data", None)
                if anim_data:
                    anim_data.drivers.remove(fcurve)
                else: # Action
                    idblock.fcurves.remove(fcurve)
            
            if print_results:
                print("#########################")
                print("# FCurves that were affected by the removed custom properties:")
                print("\n".join(print_results))
                print("#########################")
    ...
    @classmethod
    def delete(cls, idnames):
        cls.remove(None, idnames)
    
    @classmethod
    def rename(cls, idnames, name, objs=None):
        drivers_options = addon.preferences.custom_props_drivers
        update_variables = ('RENAME' in drivers_options)
        ...
        for container in cls.candidates(objs):
            for key, value in container.items():
                ...
                new_key = names[idnames.index(idname)]
                
                if update_variables: queue.add(container, key, new_key)
                
                IDProp.rename(container, key, new_key)
                
                rename_map.setdefault(idname, []).append(cls.idname(container, new_key, grouping))
        
        if update_variables:
            for fcurve_info, target_info in BlUtil.FCurve.find_dependencies(queue.entries):
                new_key = queue.entries[(target_info["id"], target_info["data_path"])]
                new_key = f'["{new_key}"]'
                
                target = target_info.get("target")
                if target:
                    target.data_path = new_key
                else:
                    fcurve = fcurve_info["fcurve"]
                    fcurve.data_path = new_key
    ...
    @classmethod
    def reset(cls, objs, idnames):
        keys = [cls._remove_type(idname) for idname in idnames]
        for container in cls.candidates(objs):
            for key in keys:
                IDProp.reset(container, key)
            # Trigger re-evaluation of drivers
            container.id_data.update_tag()
        trigger_redraw(update=True)
    ...
    def draw_prefs(cls, layout, context):
        ...
        layout.prop(addon_prefs, "custom_props_ui_data", text="UI data")
        layout.prop_menu_enum(addon_prefs, "custom_props_grouping", text="Grouping")
        layout.prop_menu_enum(addon_prefs, "custom_props_drivers", text="FCurve/Driver")
    ...
    def extra_operators(cls):
        ...
        basic_types = (bool, int, float, str, None.__class__)
        convertible_types = (bool, int, float, str)
        numeric_types = (bool, int, float)
        py_item_types = (list, dict, str)
        
        def make_edit_info():
            return dict(type=set(), size=set(), overridable=set(), description=set(), subtype=set(),
                min=set(), max=set(), soft_limits=set(), soft_min=set(), soft_max=set(),
                step=set(), precision=set(), default=[{}], value=[{}])
        
        def gather_infos(objs, keys):
            info = make_edit_info()
            
            def add_from_metadata(info, metadata, name):
                entry = metadata.get(name)
                if entry is not None: info[name].add(entry)
                return entry
            
            def add_array_item(variants, data):
                # If 1st-level data of different types are numerically the same,
                # it's convenient to treat them as having the same value
                ...
                if isinstance(data, bool):
                    ...
                    try:
                        data_int = int(data)
                        if data_int == data: data = data_int
                    except: # nan, inf, large values
                        pass
                
                key = (str(data) if isinstance(data, (int, float)) else data)
                key = json.dumps(key, sort_keys=True)
                variants[key] = data
            
            def add_array(info_array, size, values_array):
                if values_array is None: return
                
                if not size:
                    add_array_item(info_array[0], values_array)
                    return
                
                for i, item in enumerate(values_array):
                    if i >= len(info_array): info_array.append({})
                    add_array_item(info_array[i], item)
            ...
            for container in cls.candidates(objs):
                for key, value in container.items():
                    ...
                    value = IDProp.normalize(value)
                    metadata = IDProp.get_metadata(container, key)
                    overridable = IDProp.get_overridable(container, key)
                    
                    info["type"].add(type_name)
                    info["size"].add(size)
                    info["overridable"].add(overridable)
                    
                    add_array(info["value"], size, value)
                    
                    if not metadata: continue
                    
                    add_from_metadata(info, metadata, "description")
                    add_from_metadata(info, metadata, "subtype")
                    add_from_metadata(info, metadata, "step")
                    add_from_metadata(info, metadata, "precision")
                    
                    hard_min = add_from_metadata(info, metadata, "min")
                    hard_max = add_from_metadata(info, metadata, "max")
                    
                    if hard_min is not None:
                        soft_min = metadata.get("soft_min")
                        soft_max = metadata.get("soft_max")
                        soft_limits = (soft_min != hard_min) or (soft_max != hard_max)
                        info["soft_limits"].add(soft_limits)
                        
                        if soft_limits:
                            add_from_metadata(info, metadata, "soft_min")
                            add_from_metadata(info, metadata, "soft_max")
                    
                    add_array(info["default"], size, metadata.get("default"))
            
            return info
        
        # eval() automatically adds __builtins__ if it's not present
        # vec is just an alias for convenience
        eval_globals = {name: getattr(math, name) for name in dir(math) if not name.startswith("_")}
        eval_globals.update(math=math, numpy=np, np=np, re=re, vec=np.array)
        eval_globals.update(Vector=Vector, Matrix=Matrix, Quaternion=Quaternion, Euler=Euler, Color=Color)
        
        def make_add_edit(op_name, label, description):
            is_edit = (op_name == "edit")
            label = label.format(cls.singular) + ("(s)" if is_edit else "")
            description = description.format(cls.singular.lower())
            if is_edit: description += "(Click: in selected objects, Shift+Click: globally)"
            ...
            shared_data = {"active_op": None, "last_type": ""}
            
            def optional(name="", tooltip=None, update=None, min=-inf, max=inf,
                         soft_min=None, soft_max=None, precision=2, **variants):
                ...
                def update(self, context):
                    self.use = True
                    if extra_update: extra_update(self, context)
                
                def set_soft_limits(kwargs):
                    kwargs["soft_min"] = (kwargs["min"] if soft_min is None else soft_min)
                    kwargs["soft_max"] = (kwargs["max"] if soft_max is None else soft_max)
                
                for k, v in variants.items():
                    kwargs = {}
                    
                    vt = type(v) # not using isinstance because bool is decendant of int
                    if vt == int:
                        kwargs["min"] = (math.ceil(min) if min > -0x80000000 else -0x80000000)
                        kwargs["max"] = (math.ceil(max) if max < 0x7FFFFFFF else 0x7FFFFFFF)
                        set_soft_limits(kwargs)
                    elif vt == float:
                        kwargs["min"] = min
                        kwargs["max"] = max
                    ...
                    res[k] = v | prop(name, tooltip, update=update, **kwargs)
                
                return res
            
            def set_optional(p, use, **variants):
                for k, v in variants.items():
                    setattr(p, k, v)
                p.use = use
            
            def ensure_size(coll, n):
                while len(coll) < n:
                    item = coll.add()
                    item.use = True
            
            def ensure_item(coll, i):
                ensure_size(coll, i+1)
                return (coll[i] if i >= 0 else None)
            
            def size_update(self, context):
                active_op = shared_data["active_op"]
                ensure_size(active_op.value, self.v)
                ensure_size(active_op.default, self.v)
                page_update(active_op, context)
            
            def page_update(self, context):
                page_count = max(math.ceil(self.size.v / items_per_page), 1)
                if self.page > page_count: self.page = page_count
            
            def soft_limits_update(self, context):
                if recursion_lock: return
                with recursion_lock:
                    active_op = shared_data["active_op"]
                    if self.v: return
                    set_optional(active_op.soft_min, active_op.soft_min.use, f=active_op.min.f, i=active_op.min.i)
                    set_optional(active_op.soft_max, active_op.soft_max.use, f=active_op.max.f, i=active_op.max.i)
            
            def soft_min_max_update(self, context):
                if recursion_lock: return
                with recursion_lock:
                    active_op = shared_data["active_op"]
                    soft_limits = (active_op.soft_min != active_op.min) or (active_op.soft_max != active_op.max)
                    active_op.soft_limits.v = soft_limits
            
            def min_update(self, context):
                active_op = shared_data["active_op"]
                if active_op.soft_limits.v: return
                soft_limits_use = active_op.soft_limits.use
                set_optional(active_op.soft_min, active_op.soft_min.use, f=self.f, i=self.i)
                set_optional(active_op.soft_limits, soft_limits_use, v=False)
            
            def max_update(self, context):
                active_op = shared_data["active_op"]
                if active_op.soft_limits.v: return
                soft_limits_use = active_op.soft_limits.use
                set_optional(active_op.soft_max, active_op.soft_max.use, f=self.f, i=self.i)
                set_optional(active_op.soft_limits, soft_limits_use, v=False)
            
            numeric_names = ("min", "max", "soft_min", "soft_max", "step")
            shared_names = ("default", "value")
            
            def copy_values(active_op, names, old_type, new_type):
                if (new_type == old_type) or (not old_type): return
                
                for name in names:
                    attr = getattr(active_op, name)
                    
                    for p in ([attr] if hasattr(attr, "use") else attr):
                        if not p.use:
                            p.property_unset(new_type)
                            continue
                        
                        use = p.use
                        old_value = getattr(p, old_type)
                        new_value = getattr(p, new_type)
                        dst_type = type(new_value)
                        
                        if new_type == "p":
                            new_value = repr(old_value)
                        else:
                            if old_type == "p":
                                old_value = active_op.apply_edits_python(old_value, None)
                            
                            converter = IDProp.converters.get((type(old_value), dst_type))
                            if converter: new_value = converter(old_value, new_value)
                            
                            # Happens when failed to convert
                            if new_value is None: new_value = dst_type()
                        
                        setattr(p, new_type, new_value)
                        p.use = use
            
            def type_update(self, context):
                active_op = shared_data["active_op"]
                old_type = shared_data["last_type"]
                new_type = active_op.type_codes[active_op.type_name.v]
                shared_data["last_type"] = new_type
                
                if (new_type == old_type) or (not old_type): return
                
                copy_values(active_op, shared_names, old_type, new_type)
                
                if old_type == "f":
                    copy_values(active_op, numeric_names, old_type, "i")
                elif old_type == "i":
                    copy_values(active_op, numeric_names, old_type, "f")
            
            @simple_operator(f"{cls.prefix}.{op_name}", label, description, poll=batch_poll)
            class BatchOp:
                globally: False | prop()
                idname: "" | prop("")
                size: optional(v=0, update=size_update, min=0, soft_max=32) | prop()
                overridable: optional(v=False) | prop()
                description: optional(v="") | prop()
                description_dummy: "" | prop("", "Python objects (and string arrays) can't have metadata")
                subtype: optional(
                    f='NONE' | prop(items=IDProp.float_subtypes),
                    fa='NONE' | prop(items=IDProp.float_array_subtypes),
                ) | prop()
                min: optional(f=0.0, i=0, update=min_update) | prop()
                max: optional(f=1.0, i=1, update=max_update) | prop()
                soft_limits: optional(v=False, update=soft_limits_update) | prop()
                soft_min: optional(f=0.0, i=0, update=soft_min_max_update) | prop()
                soft_max: optional(f=1.0, i=1, update=soft_min_max_update) | prop()
                step: optional(f=1.0, i=1, min=0.001, precision=3) | prop()
                precision: optional(f=3, i=0, min=0, max=8) | prop()
                default: [optional(f=0.0, i=0, b=False, s="", p="")] | prop()
                value: [optional(f=0.0, i=0, b=False, s="", p="")] | prop()
                page: 1 | prop(min=1, update=page_update)
                ...
                def get_type_codes(self):
                    t = self.type_codes.get(self.type_name.v, "")
                    st = self.subtype_codes.get((self.type_name.v, (self.size.v > 0)), "")
                    return t, st
                
                def cleanup(self):
                    self.idname = ""
                    shared_data["active_op"] = None
                    shared_data["last_type"] = ""
                
                def execute(self, context):
                    objs = (None if self.globally else cls.selected_objects(context))
                    
                    if is_edit:
                        keys = {cls._remove_type(idname) for idname in cls.selected_idnames(context)}
                    elif not self.idname:
                        self.report({'ERROR_INVALID_INPUT'}, "Property name is empty")
                        return {'CANCELLED'}
                    else:
                        keys = [self.idname]
                    
                    t, st = self.get_type_codes()
                    
                    metadata = {}
                    
                    def assign_metadata(name, k):
                        p = getattr(self, name)
                        if p.use: metadata[name] = getattr(p, k, None)
                    
                    assign_metadata("description", "v")
                    assign_metadata("subtype", st)
                    assign_metadata("min", t)
                    assign_metadata("max", t)
                    assign_metadata("step", t)
                    assign_metadata("precision", t)
                    
                    if self.soft_limits.v:
                        assign_metadata("soft_min", t)
                        assign_metadata("soft_max", t)
                    
                    size = max(self.size.v, 1)
                    default = []
                    for i, item in enumerate(self.default):
                        if i >= size: break
                        default.append(getattr(item, t, None) if item.use else None)
                    metadata["default"] = (default[0] if self.size.v == 0 else default)
                    
                    with UndoBlock(label):
                        for container in cls.candidates(objs):
                            for key in keys:
                                if is_edit:
                                    if key not in container: continue
                                    value = container[key]
                                    type_info = IDProp.get_type_info(value, True)
                                    value = IDProp.normalize(value)
                                else:
                                    value = None
                                    type_info = ("python", 0)
                                
                                container[key] = self.apply_edits(value, type_info, t)
                                
                                # We need to check the converted container[key], not what was assigned to it
                                if metadata and (type(container[key]) in IDProp.types_with_metadata):
                                    _metadata = metadata
                                    if not self.soft_limits.v:
                                        prev_metadata = IDProp.get_metadata(container, key)
                                        _metadata = dict(metadata)
                                        _metadata["soft_min"] = _metadata.setdefault("min", prev_metadata.get("min"))
                                        _metadata["soft_max"] = _metadata.setdefault("max", prev_metadata.get("max"))
                                    IDProp.set_metadata(container, key, _metadata, sanitize=True)
                                
                                if self.overridable.use:
                                    IDProp.set_overridable(container, key, self.overridable.v)
                ...
                def coerce_value(self, value):
                    if isinstance(value, basic_types): return value
                    
                    if hasattr(value, "keys") and hasattr(value, '__getitem__'):
                        return {str(k): self.coerce_value(v) for k, v in value.items()}
                    
                    # IDProp lists cannot contain None, only numeric data or str/dict
                    result = [self.coerce_value(v) for v in value if v is not None]
                    numbers_count = sum(isinstance(v, numeric_types) for v in result)
                    if (numbers_count == len(result)) or (numbers_count == 0): return result
                    
                    # Arrays cannot contain both numeric (bool, int, float) and object types (str, dict)
                    # To avoid getting the user confused, just convert all numeric values to strings
                    for i, v in enumerate(result):
                        if isinstance(v, numeric_types): result[i] = repr(v)
                    
                    return result
                
                def apply_edits_python(self, py_expr, value):
                    py_expr = py_expr.strip()
                    if not py_expr: return value
                    
                    eval_locals = {"value": value, "val": value, "v": value}
                    
                    try:
                        value = eval(py_expr, eval_globals, eval_locals)
                    except Exception as exc:
                        print(f"Error when evaluating expression {repr(py_expr)}:\n{exc}")
                        return value
                    
                    try:
                        return self.coerce_value(value)
                    except Exception as exc:
                        print(f"Error when coercing the result of {repr(py_expr)}:\n{exc}")
                        return value
                
                def apply_edits(self, value, type_info, t):
                    type_name, size = type_info
                    
                    new_type = (self.type_name.v if self.type_name.use else type_name)
                    new_size = (self.size.v if self.size.use else size)
                    
                    default_value = self.default_values[new_type]
                    dst_type = type(default_value)
                    dst_convertible = (dst_type in convertible_types)
                    
                    def get_item(i):
                        value_at_i = ((value[i] if i < size else default_value) if size > 0 else value)
                        if i >= len(self.value): return value_at_i
                        
                        item = self.value[i]
                        if not item.use: return value_at_i
                        
                        if t != "p": return getattr(item, t)
                        
                        # if new_size == 0, process the value as a whole (even it it's an array)
                        input_value = (value_at_i if new_size > 0 else value)
                        return self.apply_edits_python(getattr(item, t), input_value)
                    
                    new_value = []
                    for i in range(max(new_size, 1)):
                        item = get_item(i)
                        src_type = type(item)
                        src_convertible = (src_type in convertible_types)
                        
                        if not (dst_convertible and src_convertible):
                            if not dst_convertible:
                                if (new_size == 0) or (src_type in py_item_types):
                        ...
                        converter = IDProp.converters.get((src_type, dst_type))
                        new_value.append(converter(item, None))
                    
                    return (new_value[0] if new_size == 0 else new_value)
                ...
                def invoke(self, context, event):
                    def reset_values(p):
                        for k in "vfibsp":
                            if hasattr(p, k): p.property_unset(k)
                    
                    def set_value(p, values, sub_name):
                        reset_values(p)
                        p.use = (len(values) <= 1)
                        if len(values) != 1: return
                        if not hasattr(p, sub_name): return
                        value = next(iter(values))
                        if sub_name == "p":
                            value = repr(value)
                        else:
                            src_type = type(value)
                            dst_type = type(getattr(p, sub_name))
                            converter = IDProp.converters.get((src_type, dst_type))
                            if not converter: return
                            value = converter(value, None)
                        setattr(p, sub_name, value)
                    
                    def set_items(coll, source, sub_name, size):
                        for p in coll:
                            reset_values(p)
                            p.use = True
                        
                        for i in range(max(size, 1)):
                            p = ensure_item(coll, i)
                            if len(source) <= i: return
                            set_value(p, source[i].values(), sub_name)
                    
                    def most_universal_type(types):
                        if not types: return 'python'
                        if "python" in types: return 'python'
                        if ("string" in types) or ("string[]" in types): return 'string'
                        if ("float" in types) or ("float[]" in types): return 'float'
                        if ("int" in types) or ("int[]" in types): return 'int'
                        if ("bool" in types) or ("bool[]" in types): return 'bool'
                        return 'python' # just in case
                    
                    self.globally = event.shift
                    
                    # Note: shared_data must be initialized before we set
                    # any operator properties (due to the update callbacks)
                    shared_data["active_op"] = self
                    shared_data["last_type"] = ""
                    ...
                    if is_edit:
                        keys = {cls._remove_type(idname) for idname in cls.selected_idnames(context)}
                        info = gather_infos(objs, keys)
                        self.idname = ", ".join(sorted(keys))
                    else:
                        for container in cls.candidates(objs):
                            self.existing_keys.update(container.keys())
                    ...
                    if not types:
                        self.report({'ERROR_INVALID_CONTEXT'}, "Selected objects don't contain these properties")
                        return {'CANCELLED'}
                    
                    self.type_name.v = most_universal_type(types)
                    self.type_name.use = (len(types) == 1)
                    
                    sizes = info["size"]
                    size = max(sizes)
                    self.size.v = size
                    self.size.use = (len(sizes) == 1)
                    
                    t, st = self.get_type_codes()
                    
                    shared_data["last_type"] = t
                    
                    set_value(self.overridable, info["overridable"], "v")
                    set_value(self.description, info["description"], "v")
                    set_value(self.subtype, info["subtype"], st)
                    set_value(self.min, info["min"], t)
                    set_value(self.max, info["max"], t)
                    set_value(self.soft_min, info["soft_min"], t)
                    set_value(self.soft_max, info["soft_max"], t)
                    set_value(self.soft_limits, info["soft_limits"], "v") # after soft min/max
                    set_value(self.step, info["step"], t)
                    set_value(self.precision, info["precision"], t)
                    set_items(self.default, info["default"], t, size)
                    set_items(self.value, info["value"], t, size)
                ...
                def draw(self, context):
                    ...
                    idname_exists = (self.idname in self.existing_keys)
                    is_numeric = t in ("f", "i", "b")
                    is_string = (t == "s") and (self.size.v == 0)
                    has_limits = t in ("f", "i")
                    has_subtype = (t == "f")
                    has_precision = (t == "f")
                    
                    def draw_row(p, sub_name, text="", align=False, **kwargs):
                        if not hasattr(p, sub_name): return
                        with layout.row(align=align)(alert=(not p.use)):
                            layout.prop(p, sub_name, text=text, **kwargs)
                    
                    split_factor = 0.25
                    
                    # TODO: if in "add mode" and name exists among the selected containers, show as alert
                    
                    with layout.split(factor=split_factor):
                        layout.label(text=("Name(s):" if is_edit else "Name:"))
                        with layout.row()(enabled=(not is_edit), alert=idname_exists, activate_init=True):
                            layout.prop(self, "idname", text="")
                    
                    with layout.split(factor=split_factor):
                        layout.label(text="Data:")
                        with layout.row(align=True):
                            draw_row(self.type_name, "v", "", align=True)
                            draw_row(self.size, "v", "Dimension", align=True)
                    
                    if has_subtype:
                        # Custom properties only have float subtypes for now
                        with layout.split(factor=split_factor):
                            layout.label(text="Subtype:")
                            draw_row(self.subtype, st, "")
                    ...
                    if is_string:
                        with layout.column(align=True):
                            with layout.split(factor=split_factor):
                                ...
                                with layout.row(align=True):
                                    draw_row(self.value[0], t, "", align=True)
                            with layout.split(factor=split_factor):
                                layout.label(text="Default:")
                                with layout.row(align=True):
                                    draw_row(self.default[0], t, "", align=True)
                    elif self.size.v < 1:
                        with layout.row(align=True):
                            if is_numeric:
                                draw_row(self.value[0], t, "Value", align=True)
                                draw_row(self.default[0], t, "Default", align=True)
                            else:
                                draw_row(self.value[0], t, "Value", align=True)
                    else:
                        page_count = max(math.ceil(self.size.v / items_per_page), 1)
                        page_offset = (self.page - 1) * items_per_page
                        i_min = page_offset
                        i_max = min(page_offset + items_per_page, self.size.v)
                        
                        with layout.column(align=True):
                            for i in range(i_min, i_max):
                                with layout.row(align=True):
                                    if is_numeric:
                                        draw_row(self.value[i], t, f"Item {i+1}", align=True)
                                        draw_row(self.default[i], t, "Default", align=True)
                                    else:
                                        draw_row(self.value[i], t, f"Item {i+1}", align=True)
                        
                        if page_count > 1:
                            layout.prop(self, "page", text="Page")
                    ...
                    if has_limits:
                        with layout.column(align=True):
                            with layout.split(factor=split_factor):
                                ...
                                with layout.row(align=True):
                                    draw_row(self.min, t, "Min", align=True)
                                    draw_row(self.max, t, "Max", align=True)
                            
                            with layout.split(factor=split_factor):
                                draw_row(self.soft_limits, "v", "Soft", toggle=True)
                                with layout.row(align=True)(active=self.soft_limits.v):
                                    draw_row(self.soft_min, t, "Min", align=True)
                                    draw_row(self.soft_max, t, "Max", align=True)
                        
                        with layout.split(factor=split_factor):
                            layout.label(text="")
                            with layout.row(align=True):
                                draw_row(self.step, t, "Step", align=True)
                                with layout.row(align=True)(enabled=has_precision):
                                    draw_row(self.precision, t, "Precision", align=True)
                    
                    with layout.split(factor=split_factor):
                        layout.label(text="Description:")
                        if is_numeric or is_string:
                            draw_row(self.description, "v")
                        else:
                            with layout.row()(enabled=False):
                                layout.prop(self, "description_dummy", text="")
                    
                    with layout.row():
                        draw_row(self.overridable, "v", "Library Overridable")
            
            main_overrides.add(op_name)
        
        make_add_edit("add_new", "New {0}", "Add a new {0}")
        make_add_edit("edit", "Edit {0}", "Adjust {0} values and settings")
        
        @simple_operator(f"{cls.prefix}.reset", f"Reset {cls.plural}",
            f"Reset {cls.plural.lower()} to default value(s) (Click: in selected objects, Shift+Click: globally)",
            poll=batch_poll)
        class BatchOp:
            globally: False | prop()
            def execute(self, context):
                objs = (None if self.globally else cls.selected_objects(context))
                with UndoBlock(f"Reset {cls.plural}"):
                    cls.reset(objs, cls.selected_idnames(context))
                trigger_redraw()
                return {'FINISHED'}
            def invoke(self, context, event):
                self.globally = event.shift
        ...
        def extras_main(layout, context):
            layout.operator(f"{cls.prefix}.edit", text="Edit", icon='OPTIONS')
            layout.operator(f"{cls.prefix}.reset", text="Reset", icon='RECOVER_LAST')
        
        return {"extras_main":extras_main, "main_overrides":main_overrides}

make_batch_ops(BatchOps_CustomProps, "Custom Prop")
...
class BatchOps_NodeEditor_NodeGroups(BatchOps):
    ...
    def enumerate(cls, objs, callbacks=(), context=None, visible_only=False, raw=False):
        for node_group in bpy.data.node_groups:
            if node_group.bl_idname != cls.node_type: continue
...
class BatchOps_NodeEditor_NodeGroups_Geometry(BatchOps_NodeEditor_NodeGroups):
    node_type = 'GeometryNodeTree'

make_batch_ops_node_editor(BatchOps_NodeEditor_NodeGroups_Geometry, "Node Group", tree_type='GeometryNodeTree', has_prefs=False, can_filter=False)
...
class ScreenProps:
    ...
    show_context_menu_: copy_prop(BatchOpsPreferences.__annotations__["show_context_menu"])
    ...
    show_context_menu: copy_prop(BatchOpsPreferences.__annotations__["show_context_menu"])
    ...
    def update_panels(cls, new_panels):
        for screen in bpy.data.screens:
            ...
            screen.batch_ops.show_context_menu |= new_panels
    ...
    def setup(cls):
        def init(self):
            ...
            self.show_context_menu_ = addon.preferences.show_context_menu
        ...
        def _get(self):
            if not self.get("init"): init(self)
            return BlRna.enum_to_int(self, "show_context_menu_")
        def _set(self, value):
            self.show_context_menu_ = BlRna.enum_from_int(self, "show_context_menu_", value)
        BpyProp(cls.__annotations__["show_context_menu"]).update(get=_get, set=_set)
...
def register_keymaps():
    ...
    kmi = km.keymap_items.new('batch_ops.hover_processor', 'MOUSEMOVE', 'ANY')

def unregister_keymaps():
    addon.unregister_keymaps()

def register():
    addon.runtime.hover_idnames = {}
    ...
    pcoll.load("blank", os.path.join(icons_dir, "item_indicator_blank.png"), 'IMAGE')
    pcoll.load("hover", os.path.join(icons_dir, "item_indicator_hover.png"), 'IMAGE')
    pcoll.load("selection", os.path.join(icons_dir, "item_indicator_selection.png"), 'IMAGE')
    pcoll.load("selection_hover", os.path.join(icons_dir, "item_indicator_selection_hover.png"), 'IMAGE')
    preview_collections["item_indicator"] = pcoll
