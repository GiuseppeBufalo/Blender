...
class MergeToLastPoint(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if same_spline_merge:
            ...
            new_points, spline_data = self.get_other_spline_merge_new_points(data, last, other_spline_merge, debug=False)
        ...
        if same_spline_merge or other_spline_merge:
            ...
            if couldnt_merge:
                popup_message("Some, but not not all of the selected points in the selection could be merged", title="Illegal Selection")

            return {'FINISHED'}

        else:
            popup_message(["Nothing could be merged!", "Make a selection, that - once merged - doesn't create a self-intersecting spline, or a cyclic loop"], title="Illegal Selection")
...
class MergeToCenter(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if separated:
            ...
            if len(separated) > 1:
                ...
                new_points, spline_data, sidx1, sidx2 = self.get_other_spline_merge_new_points(data, separated, debug=False)
    ...
    def get_other_spline_merge_new_points(self, data, separated, debug=False):
        ...
        if len(separated) == 2:
            ...
            if sel1[0] == 0 or sel1[-1] == len(spline_data1['points']) - 1:
                ...
                if sel2[0] == 0 or sel2[-1] == len(spline_data2['points']) - 1:
                    ...
                    popup_message("To Center Merge points across two splines, you need to have a continuous selection at either end of the two splines.", title="Illegal Selection")
            else:
                popup_message("To Center Merge points across two splines, you need to have a continuous selection at either end of the splines", title="Illegal Selection")
        else:
            popup_message("You can't merge spline points accross more than 2 splines", title="Illegal Selection")
