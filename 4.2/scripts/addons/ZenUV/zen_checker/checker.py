...
class ZUVChecker_OT_CheckerToggle(bpy.types.Operator):
    ...
    def poll(cls, context):
        return context.selected_objects and context.area.type == 'VIEW_3D'
    ...
    def execute(self, context):
        ...
        if materials_with_overrider:  # Case checker exist in currently selected objects - Turn Off Checker
            ...
            switch_shading_style(context, context.scene.zen_uv_checker.prev_color_type, switch=False)

        else:  # Else - Turn On Checker
            context.scene.zen_uv_checker.prev_color_type = get_current_shading_style(context)
            repair_zen_generic_mat()
            for obj in sel_objects:
                implement_zen_overrider(context, obj, GlobalOverrider)
            zen_checker_image_ensure(context)
