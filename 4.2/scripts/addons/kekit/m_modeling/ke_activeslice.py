...
class KeActiveSlice(Operator):
    ...
    cut : EnumProperty(items=[
        ("SLICE", "Slice", "Cuts geo along the cutter plane", "", 1),
        ("OUTER", "Delete Outer Side", "Removes geo on the positive side of the cutter plane normal", "", 2),
        ("INNER", "Delete Inner Side", "Removes geo on the negative side of the cutter plane normal", "", 3)],
        name="Cut Type",
        default="SLICE"
    )
    ...
    def draw(self, context):
        ...
        layout.separator()
        layout.prop(self, "cut", expand=True)
        ...
        for obj in sel_objects:
            ...
            if self.target_geo:
                ...
                geom = bmesh.ops.bisect_plane(
                    ...
                    plane_co=plane_p,
                    plane_no=plane_n,
                    clear_inner=rem_inner,
                    clear_outer=rem_outer
                )
                new_edges = [i for i in geom['geom_cut'] if isinstance(i, bmesh.types.BMEdge)]
                if obj == active_obj:
                    # FILTER W. EXCLUDE LIST, BETTER THAN 'GEOM' RETURN?
