import bpy
import os
from .. utils.registration import get_prefs, get_path, set_new_decal_index, get_addon, get_addon_prefs, is_library_in_assetspath
...
from .. utils.system import get_safe_filename, printd
from .. import bl_info
...
class PanelDECALmachine(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if dm.debug or bpy.ops.machin3.debug_decal.poll():
            column.separator()
    ...
    def draw_defaults(self, dm, wm, layout):
        ...
        r = row.row(align=True)
        r.prop(dm, "alpha_blendmode", expand=True)
    ...
    def draw_debug(self, context, dm, layout):
        ...
        if bpy.ops.machin3.debug_decal.poll():
            row = col.row(align=True)
            row.scale_y = 1.5
            row.operator("machin3.debug_decal", text="Debug Decal")
        ...
        if dm.debug:
            if context.scene.userdecallibs:
                ...
                col.separator()
                col.label(text="Create a new Library or Unlock an existing one.", icon_value=get_icon('error'))
                col.separator()
...
class PanelDecalCreation(bpy.types.Panel):
    ...
    @classmethod
    def poll(cls, context):
        return get_prefs().show_sidebar_panel and context.scene.DM.register_panel_creation
...
class PanelDecalExport(bpy.types.Panel):
    ...
    @classmethod
    def poll(cls, context):
        return get_prefs().show_sidebar_panel and context.scene.DM.register_panel_export
...
class PanelUpdateDecals(bpy.types.Panel):
    ...
    @classmethod
    def poll(cls, context):
        return get_prefs().show_sidebar_panel and context.scene.DM.register_panel_update
    ...
    def draw(self, context):
        ...
        draw_pil_warning(box, needed="for library update")

        self.draw_legacy_update(box)

        self.draw_update_library(context, dm, box)

        self.draw_update_blend_file(box)
    ...
    def draw_update_blend_file(self, layout):
        ...
        if decal or sheet:
            box = layout.box()
            column = box.column()
            column.label(text=f"Legacy Assets in Current File")
            ...
            if decal:
                for version, decals in sorted(decal.items()):
                    b = box.box()
                    b.label(text=f"Decal Version {version}")
            ...
            if sheet:
                for version, sheets in sorted(sheet.items()):
                    b = box.box()
                    b.label(text=f"Trimsheet Version {version}")
...
class PanelHelp(bpy.types.Panel):
    ...
    @classmethod
    def poll(cls, context):
        return get_prefs().show_sidebar_panel and context.scene.DM.register_panel_help
