...
from . utils.registration import get_prefs, get_version_files, get_version_filename_from_blender, get_version_as_float, get_version_from_filename, get_version_from_blender, is_library_trimsheet, is_library_corrupted, is_library_in_assetspath
...
from . utils.batch import switch_alpha_blendmode, change_ao_strength, invert_infodecals, switch_edge_highlights, toggle_coat, toggle_pack_images, toggle_material_visibility, toggle_texture_visibility, toggle_nodetree_visibility, toggle_decaltype_collection_visibility, toggle_decalparent_collection_visibility
...
from . items import create_type_items, create_decaltype_items, create_infotype_items, create_infotext_align_items, create_trimtype_items, update_library_version_items, update_library_version_30_items, update_library_version_40_items
from . items import decaltype_items, texturetype_items, pack_images_items, trimtexturetype_items, atlastexturetype_items
from . items import export_type_items, create_atlascreation_type_items, create_atlas_type_items, create_atlas_mode_items, create_atlas_prepack_items, create_atlas_trim_sort_items, exporttexturetype_items, create_atlas_size_mode_items, export_atlas_model_format_items, override_preset_items, override_preset_mapping, coat_items
    ...
    def update_material_override(self, context):
        ...
        if self.material_override:
                        ...
                        if mat.DM.isdecalmat:
                            ...
                            if dg:
                                ...
                                for output in unconnected_outputs:
                                    ...
                                    if bpy.app.version >= (4, 0, 0) and self.coat == 'UNDER' and not self.material_override_decal_subsets:
            ...
            if empty:
                ...
                emptyobjs = [obj for obj in bpy.data.objects if empty.name in obj.data.materials and len(set([mat for mat in obj.data.materials if mat])) == 1]
            ...
            if dg:
                ...
                for out in group.outputs:
                    ...
                    if subset_link and not self.material_override_decal_subsets:
                        ...
                        if bpy.app.version >= (4, 0, 0) and self.coat == 'UNDER' and out.name.startswith('Coat '):
                            continue
    ...
    def update_updatelibrarypath(self, context):
        ...
        if self.updatelibrarypath:
            ...
            if len(versions) == 1:
                ...
                elif get_version_as_float(get_version_from_filename(versions[0])) > get_version_as_float(get_version_from_blender()):
                    popup_message([f"The chosen library at '{sourcepath}'",
