...
import os
from . registration import get_prefs, get_templates_path, get_version_as_float, shape_version, get_version_from_blender, is_decal_registered, is_trimsheet_registered
...
def get_legacy_materials(force=False, debug=False):
    ...
    if force or legacy_dict is None:
        ...
        decalmats = [mat for mat in bpy.data.materials if mat.DM.isdecalmat and not 'LIBRARY_DECAL' in mat.name]
...
def is_legacy_material(mat, debug=False):
    expected_version = get_version_from_blender(use_float=True)
        ...
        for maptype in sheetmaps:
            ...
            if node:
                ...
                if maptype == 'HEIGHT':
                    ...
                    if maptype == 'EMISSION' and bpy.app.version >= (4, 0, 0):
                        i = "Emission Color"
                    else:
                ...
                if node.image:
                    ...
                    if new_node:
                        ...
                        if newtsg and nodetype not in ['ALPHA', 'SUBSET', 'MATERIAL2']:
                            if nodetype == 'EMISSION' and bpy.app.version >= (4, 0, 0):
                                i = "Emission Color"
                            else:
...
def match_material(decalobj, decalmat, matchmatname=None, matchmat2name=None, matchsubname=None, debug=False):
    ...
    if bpy.app.version >= (4, 0, 0) and bpy.context.scene.DM.coat == 'UNDER' and decalmat.DM.decaltype in ['SUBSET', 'PANEL']:
    ...
    transmission = max([value for d in decaldict for key, value in d.items() if key.endswith('Transmission' if bpy.app.version < (4, 0, 0) else 'Transmission Weight')])
    ...
    if pbr:
        ...
        if matchedmats:
            ...
            if tsg:
                ...
                trimsheetmat.use_screen_refraction = pbrdict.get('Transmission' if bpy.app.version < (4, 0, 0) else 'Transmission Weight') > 0
...
def create_override_node_tree():
    ...
    if bpy.app.version >= (4, 0, 0):
        ...
        socket = override.outputs.new('NodeSocketColor','Base Color')

        socket = override.outputs.new('NodeSocketFloat','Metallic')
        socket.min_value = 0
        socket.max_value = 1

        socket = override.outputs.new('NodeSocketFloat','Roughness')
        socket.min_value = 0
        socket.max_value = 1

        socket = override.outputs.new('NodeSocketFloat','IOR')
        socket.min_value = 1
    ...
    if out:
        for name, value in d.items():
            ...
            if i:
                ...
                if i.links:
                    link = i.links[0].from_socket.default_value = d[name]
