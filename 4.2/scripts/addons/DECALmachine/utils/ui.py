...
def draw_version_warning(layout, version):
    if bpy.app.version < version:
        ...
        layout.separator()
        layout.separator()
        layout.label(text=f"Official Support for Blender versions older than {str(b3d_version)} LTS has ended. Please update Blender to ensure DECALmachine {addon_version} works reliably.", icon_value=get_icon('error'))
        layout.separator()
        layout.separator()
        layout.separator()
        layout.separator()
