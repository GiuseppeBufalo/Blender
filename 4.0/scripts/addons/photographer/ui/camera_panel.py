...
class PHOTOGRAPHER_PT_Panel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene.camera != bpy.context.active_object:
            ...
            if main_cam:
                if main_cam.data == context.camera:
...
def exposure_header(self, context, use_scene_camera=False):
    layout = self.layout
    if use_scene_camera:
        cam = context.scene.camera
        settings = cam.data.photographer
...
def exposure_panel(self, context, settings, prop_panel, guide, image_editor):
        ...
        if guide:
            ...
            row.alignment = 'RIGHT'
            ev = calc_exposure_value(settings, context)
            ev_guide = update_exposure_guide(settings, context, ev)
        ...
        if context.scene.render.engine in {'CYCLES','BLENDER_EEVEE'}:
            alt = bpy.context.preferences.addons[addon_name].preferences.auto_light_threshold
            hltw = bpy.context.preferences.addons[addon_name].preferences.hide_light_threshold_warning
...
class PHOTOGRAPHER_PT_ViewPanel_Exposure(bpy.types.Panel):
    ...
    def draw_header(self, context):
        settings = context.scene.camera.data.photographer
        exposure_header(self,context,True)
...
def whitebalance_header(self, context, use_scene_camera=False):
    layout = self.layout
    if use_scene_camera:
        cam = context.scene.camera
        settings = cam.data.photographer
...
class PHOTOGRAPHER_PT_ViewPanel_WhiteBalance(bpy.types.Panel):
    ...
    def draw_header(self, context):
        whitebalance_header(self,context,True)
...
def resolution_header(self, context, use_scene_camera=False):
    layout = self.layout
    if use_scene_camera:
        cam = context.scene.camera
        settings = cam.data.photographer
        ...
        settings = cam.photographer    
    main_cam = context.scene.photographer.main_camera
    layout.prop(settings, "resolution_enabled", text="")
    if main_cam != 'NONE':
        if main_cam != cam.name:
...
class PHOTOGRAPHER_PT_Panel_Resolution(bpy.types.Panel):
    ...
    def draw_header(self, context):
        resolution_header(self,context)
...
class PHOTOGRAPHER_PT_ViewPanel_Resolution(bpy.types.Panel):
    ...
    def draw_header(self, context):
        resolution_header(self,context,True)
