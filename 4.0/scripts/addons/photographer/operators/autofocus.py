...
class PHOTOGRAPHER_OT_CreateFocusPlane(bpy.types.Operator):
    ...
    def execute(self,context):
        ...
        mat.diffuse_color = bpy.context.preferences.addons[addon_name].preferences.default_focus_plane_color
        ...
        mix_rgb.inputs[2].default_value = bpy.context.preferences.addons[addon_name].preferences.default_focus_plane_color
        ...
        if event.type == 'LEFTMOUSE':
            if event.value == 'RELEASE':
                if context.space_data.type == 'VIEW_3D':
                    ...
                    result, location, object, normal = raycast(context, event, True, False, cam_obj)
                    if not result:
...
def active_cam_check(context):
    ...
    if bpy.context.preferences.addons[addon_name].preferences.show_af_buttons_pref:
        # for area in bpy.context.screen.areas:
