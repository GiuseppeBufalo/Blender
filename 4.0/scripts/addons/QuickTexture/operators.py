            ...
            # UV/BOX TOGGLE
                ...
                if self.tex_coord.label == 'Box' or self.tex_coord.label == 'UV':
                    if self.diffuse_tex:
                        ...
                        if self.mask_mapping:
                            self.mask_mapping.inputs[3].default_value[0] = 1
                            self.mask_mapping.inputs[3].default_value[1] = 1
                            self.mask_mapping.inputs[3].default_value[2] = 1
    ...
    def poll(self,context):
        ...
        if bpy.context.window_manager.my_toolqt.activelayer > 1:
            if bpy.context.window_manager.my_toolqt.selected:
                if bpy.context.window_manager.my_toolqt.ob:
                    if len(bpy.context.window_manager.my_toolqt.ob.material_slots) > 0:
                        mat = bpy.context.window_manager.my_toolqt.ob.material_slots[0].material
                        if mat:                    
                            for n in mat.node_tree.nodes:
                                if n.name.startswith('QT_Shader'):
