...
def draw_callback_view(self, context):
    ...
    if self.history:
        if self.bb_lines and self.bb_graylines and self.display[0] != "DISTANCE":
            ...
            batch = batch_for_shader(self.shader, 'LINES', {"pos": self.bb_graylines})
    ...
    if self.history:
        if self.bb_mpos and self.display[0] != "DISTANCE":
            for t, s in zip(self.bb_mpos, self.bb_lines_dist):
                if t and s:
                    blf.position(self.font_id, t[0], t[1] + self.fontsize[0], 0)
...
class KeQuickMeasure(Operator):
    ...
    def invoke(self, context, event):
        ...
        set_status_text(self.ctx, status_help)
