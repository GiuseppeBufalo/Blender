...
from .. utils.ui import init_cursor, popup_message, wrap_cursor, get_zoom_factor, init_status, finish_status, navigation_passthrough, get_mousemove_divisor, scroll_up, scroll_down
from .. utils.draw import draw_point, draw_points, draw_vector, draw_line, draw_init, update_HUD_location, draw_label
            ...
            if event.type in  ['MOUSEMOVE', 'A', 'M', 'C'] or scroll_up(event, key=True) or scroll_down(event, key=True):
                ...
                if event.type == 'MOUSEMOVE':
                    ...
                    if self.passthrough:
                        ...
                        self.factor = get_zoom_factor(context, self.mx @ self.data['active_selection_mid_point'])
                    ...
                    if self.is_tension:
                        wrap_cursor(self, context, event, y=True)
            ...
            if spline['type'] in ['POLY', 'NURBS'] and len(spline['points']) >= 3:
                ...
                if selection := verify_curve_data(self.data, 'has_active_selection'):
                    ...
                    if not verify_curve_data(self.data, 'is_active_end_selected'):
                        ...
                        popup_message(["Avoid selecting the Spline Ends", "You can use the Gap Shuffle tool to easiy move the End Points elsewhere."], title="Illegal Selection")
                else:
