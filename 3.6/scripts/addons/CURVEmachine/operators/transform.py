...
from .. utils.draw import draw_vector, draw_point, draw_init, draw_label, update_HUD_location, draw_line
from .. utils.ui import get_zoom_factor, init_cursor, init_status, finish_status
...
class SlidePoint(bpy.types.Operator):
    ...
    def modal(self, context, event):
        ...
        elif event.type not in ['TIMER_REPORT', 'INBETWEEN_MOUSEMOVE']:
    ...
    def invoke(self, context, event):
        ...
        self.mousepos = Vector((event.mouse_region_x, event.mouse_region_y))
        self.mouse_offset = self.data['point_co_2d'] - self.mousepos
    ...
    def slide_point(self, context):
        ...
        view_origin = region_2d_to_origin_3d(context.region, context.region_data, self.mousepos + self.mouse_offset)
        view_dir = region_2d_to_vector_3d(context.region, context.region_data, self.mousepos + self.mouse_offset)
