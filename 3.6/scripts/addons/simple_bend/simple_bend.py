...
class SimpleBendOperator(bpy.types.Operator):
    ...
    prop_selectonly: BoolProperty(
        name="Selected Only",
        description="Bend only selected part",
        default=False,
    )    
    ...
    prop_edge: BoolProperty(
        name="Edge Axis mode",
        description="Use lasted selected 2 edges as Rotation Axis and Orientation",
        default=False,
    )    
    ...
    prop_edge_swap: IntProperty(
        name="Swap Edge Axis",
        description="Swap the edge axis",
        default=0,
        max = 2,
        min = 0
    )    
    ...
    prop_fixed: IntProperty(
        name="Fixed mode",
        description="Index for fixed part",
        default=0,
        min = 0,
        max = 2
    )    
    ...
    def draw_text(self):
        ...
        if self.prop_edge and self.empty_edges:
            status = 'Error: Empty edges selection. Please set the axis-edge in selection history.'

        gui.txtall = [deg, status, 'Please press Esc to exit', '(Hold Shift key: 5 degree snapping)', 
            'S key: Selected only, C key: Change fixed part', 'W key: Swap Axis, Q key: Confirm']
