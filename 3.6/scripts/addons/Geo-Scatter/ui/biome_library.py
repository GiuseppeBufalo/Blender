    ...
    for element in ElementsMapping:
        ...
        with open(e.name) as f:
            try:
                ...
                print(f"    There was a .biome file that was wrongly encoded as a biome format!")
                print(f"    Path: {e.name}")
        ...
        if (e.type=="Online"):
            try:
                ...
                e.user_has = (utils.path_utils.search_for_path(keyword=user_has_file, search_folder=directories.lib_library, file_type='.userhave',)!="")
            except:
                e.user_has = False
...
def draw_biome_element(e, layout, enabled):
    ...
    down.scale_x = 0.95
    down.context_pointer_set("s5_ctxt_ptr_lib_obj", e)
...
def draw_online_element(e, layout,):
    ...
    kwargs = {"text":text_cut, "emboss":True}
    ...
    if (count==0):
        ...
        if (len(scat_win.folder_navigation)==0):
            ...
            if (not biome_in_subpaths(directories.lib_biomes)):
                ...
                ope = layout.row()
                ope.active = False
                ope.label(text=translate("Your Biome Library seems to be Empty! Install a biome pack in the Header above!"))
                
                layout.operator("scatter5.exec_line", text=translate("Discover All Biomes Available"),icon_value=cust_icon("W_SUPERMARKET")).api = "scat_win.category_manager='market' ; bpy.ops.scatter5.tag_redraw()"
                ...
                layout.operator("scatter5.reload_biome_library", text=translate("Reload Biome Library"), icon="FILE_REFRESH")
