...
class PHOTOGRAPHER_PT_Panel_Light(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return (context.light) and (engine in cls.COMPAT_ENGINES)  and (bpy.context.preferences.addons[addon_name].preferences.use_physical_lights)
    ...
    def draw(self, context):
        ...
        minus_one = row.operator("lightmixer.light_stop", text="-1")
        minus_one.light = light.name
        minus_one.factor = -1
        minus_half = row.operator("lightmixer.light_stop", text="-0.5")
        minus_half.light = light.name
        minus_half.factor= -0.5
        plus_half = row.operator("lightmixer.light_stop", text="+0.5")
        plus_half.light = light.name
        plus_half.factor = 0.5
        plus_one = row.operator("lightmixer.light_stop", text="+1")
...
class PHOTOGRAPHER_PT_EEVEE_light_distance(bpy.types.Panel):
    ...
    def poll(cls, context):
        ...
        return (light and light.type != 'SUN') and (engine in cls.COMPAT_ENGINES) and (bpy.context.preferences.addons[addon_name].preferences.use_physical_lights)
